...
import { Row, Button, Table, Checkbox } from 'antd';
...
class CreateForm extends BaseForm {
    ...
    constructor(props) {
        ...
        const stat =  Statistic.stat(dataSource || [], statisticCols);
    ...
    generataColumns = (theFormItems, pageKey, formKey) => {
      const { formType, canDelete, fixedNumber, form,  extraActions } = this.props;
      const enerateActionColumn = (formType === 'detail' && !canDelete) || fixedNumber ? [] : [{
              title: '操作',
              dataIndex: 'action',
              key: 'action',
              width: 80 + ((extraActions||[]).length || 0) * 40,
              align: 'center',
              // @ts-ignore
              render: (text, record) =>
                  record.key === 'TOTAL' ? (
                  ...
                  ) : <span>
                  {
                    ...
                    canDelete && typeof canDelete === 'function' && !canDelete(text, record) ? <span style={{color: 'rgba(0, 0, 0, 0.25)'}}>删除</span> : <a onClick = { this.doRemove.bind(this, (record || {}).key) }> 删除 </a>
                  }
                  {
                    (extraActions || []).map((it)=>{
                      if(it.isDisable === false || (typeof it.isDisable === 'function' && it.isDisable(text, record))){
                        return <span style={{color: 'rgba(0, 0, 0, 0.25)', paddingLeft: '5px'}}>{it.title}</span>;
                      }else{
                        // eslint-disable-next-line jsx-a11y/anchor-is-valid
                        return <a style={{paddingLeft: '5px'}} onClick = { it.doAction.bind(this, record) }>{it.title}</a>;
                      }
                    })
                  }
                  </span>,
    ...
    onValuesChange = (values, inputType, isExtjson, tableName, extra) => {
        ...
        this.setState({ dataSource: targetData, totalRow: {...totalRow, ...(typeof targetTotalRow === 'object' ? targetTotalRow : {}) } }, () => {
    ...
    setFieldsValue = (formValues, rowKey) => {
        ...
        this.setState({ dataSource: targetDataSource, totalRow: {...totalRow, ...(typeof targetTotalRow === 'object' ? targetTotalRow : {})} }, ()=>{
    ...
    resetForm = (targetDataSource) => {
        ...
        const { defaultEmptyRow } = this.props;
        const emptyRow = (defaultEmptyRow ? [{}] : []);
        const stat = Statistic.stat(ds || [], statisticCols) ;
        // @ts-ignore
