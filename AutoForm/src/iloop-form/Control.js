 ...
 */
function getControl(inputType, formType, fieldName, params, keys, tableName, required, valueInOptions, autocompleteOptions, setAutocompleteOptions) {
    let control = null;
    // @ts-ignore
...
function getDetailValue(initialValue, inputType) {
  //console.log('getDetailValue', initialValue, inputType);
    if (inputType === 'checkbox') {
        return <Checkbox disabled checked = { initialValue }
        />;
    }else if(['date', 'datetime'].includes(inputType)){
      return initialValue.format('YYYY-MM-DD HH:mm:ss');
    }
...
function Controls(props) {
               ...
               */
 {
                ...
                // 详情
                formType === 'detail' || inputType === 'plain' ? ( <span className = 'detailSpan' > { getDetailValue(detailInitialValue || initialValue, inputType) } </span>) : (
                    // 创建编辑控件
